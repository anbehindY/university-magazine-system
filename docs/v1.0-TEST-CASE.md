...
> **Note on Email Testing:** To test coordinator email notifications (TC-COORD-12), the tester must create a coordinator account using a **real email address** they can access. Seeded accounts use fake `@uog.com` emails that cannot receive mail. Use the Admin panel (`/users`) to create a new coordinator with your real Gmail/email, assign them to a faculty, then have a student from that faculty submit — the notification will arrive at your real inbox.
