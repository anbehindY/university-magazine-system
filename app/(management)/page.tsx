...
type DashboardData = {
  ...
  users?: { id: string; role: string; banned: boolean }[];
    ...
    <>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <Card className="border-slate-200 bg-white text-slate-900">
          ...
          <CardContent className="space-y-3">
            <Button variant="outline" className="w-full justify-between" onClick={() => router.push("/submissions")}>
...
function AdminDashboard({ data, router }: { data: DashboardData; router: ReturnType<typeof useRouter> }) {
  ...
  return (
    <>
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard title="Total Users" value={totalUsers} hint="Registered accounts" icon={Users} />
        <StatCard title="Active Users" value={activeUsers} hint="Not banned" icon={CheckCircle2} color="emerald" />
        <StatCard title="Academic Years" value={years.length} hint={`${years.filter((y) => y.isActive).length} active`} icon={CalendarClock} />
        <StatCard title="Total Submissions" value={totalSubmissions} hint="Across all faculties" icon={FileText} />
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <Card className="border-slate-200 bg-white text-slate-900">
          ...
          <CardContent className="space-y-3">
            <Button variant="outline" className="w-full justify-between" onClick={() => router.push("/users")}>
              Manage Users <ArrowRight className="h-4 w-4" />
            </Button>
            <Button variant="outline" className="w-full justify-between" onClick={() => router.push("/admin")}>
    ...
    if (r === "ADMINISTRATOR") {
      fetches.push(
        fetch("/api/admin/users")
          .then((res) => res.json())
          .then((d) => setData((prev) => ({ ...prev, users: d.users })))
          .catch(() => {})
      );
      fetches.push(
